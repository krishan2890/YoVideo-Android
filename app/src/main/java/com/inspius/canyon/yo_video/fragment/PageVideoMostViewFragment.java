 ...
 */
public class PageVideoMostViewFragment extends BaseMainFragment implements AdapterVideoActionListener {
    public static final String TAG = PageVideoMostViewFragment.class.getSimpleName();
...
//        requestGetData();
    ...
    void requestGetData() {
        ...
        if (pageNumber < 1)
            pageNumber = 1;
        RPC.requestGetVideoMostView(pageNumber, new APIResponseListener() {
            @Override
            public void onError(String message) {
                stopAnimLoading();
    ...
    void updateData(List<VideoJSON> listData) {
        List<VideoModel> events = new ArrayList<VideoModel>();
